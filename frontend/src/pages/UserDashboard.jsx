...
import StatCard from "../components/StatCard";
import ProgressBar from "../components/ProgressBar";
import socket, { joinUserRoom } from "../utils/socket"; // ✅ UPDATED
import { calculateProgress, formatEta } from "../utils/helpers";
...
const UserDashboard = () => {
  ...
  // 🔔 USER ROOM + NOTIFICATION (NEW)
  useEffect(() => {
    if (userInfo?._id) {
      console.log("Joining user room:", userInfo._id);
      joinUserRoom(userInfo._id);
    }

    socket.on("turnAlert", (data) => {
      console.log("🔥 ALERT RECEIVED:", data);

      toast.success(`⚡ Your turn is near! Only ${data.peopleAhead} ahead`);
    });
