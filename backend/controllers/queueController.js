...
// 🔥 RECALCULATE + DEBUG + NOTIFICATION
const recalculateQueueTokens = async (queueId) => {
  const queue = await Queue.findById(queueId);
  if (!queue) return;
  ...
  for (const token of tokens) {
    ...
    if (token.status === "waiting") {
      ...
      if (peopleAhead <= 2 && peopleAhead >= 0) {
        if (ioInstance) {
          ioInstance.to(token.user.toString()).emit("turnAlert", {
            peopleAhead,
          });
        }
      }
...
const getQueues = async (req, res) => {
  try {
    ...
    const queues = await Queue.find(filter).sort({ createdAt: -1 }).limit(7);
...
const joinQueue = async (req, res) => {
  try {
    ...
    if (activeAhead <= 2 && activeAhead >= 0) {
      if (ioInstance) {
        ioInstance.to(req.user._id.toString()).emit("turnAlert", {
          peopleAhead: activeAhead,
        });
      }
    }
...
const serveNextToken = async (req, res) => {
  try {
    ...
    res.json({ message: "Next token served" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
